 ...
 * 
 * @typedef {{
 *      [property: string]: QueryExtras|any
 * }} Query
 * 
 * @typedef {{
        name: string
...
export default class MangoDB {
    ...
    static async link(items, ...links) {
        try {
            const newItems = []
            let promises = [], found, counter, linked
    ...
    static find(query, item, keys) {
        return keys.every(key => {
            ...
            if (typeof query[key] === "object") {
                if (query[key]["$regexp"] && query[key]["$regexp"] instanceof RegExp && typeof item[key] === TYPE.STR) {
                    return query[key]["$regexp"].test(item[key])
                }
            }
        })
    }
    ...
    async findMany(query, options) {
        ...
        try {
            ...
            const copy = [ ...res.items ]
            if (options.$links) {
                let links = await MangoDB.link(copy, ...options.$links)
                assert(links.error === null, links.error)
                return { error: null, items: links.items }
            }
            else {
                return { error: null, items: copy }
            }
    ...
    async findOne(query, options) {
        ...
        try {
            ...
            if (options.$links) {
                let links = await MangoDB.link([copy], ...options.$links)
                assert(links.error === null, links.error)
                return { error: null, item: links.items[0] }
            }
            else {
                return { error: null, item: copy }
            }
